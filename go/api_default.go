...
func (c *DefaultApiController) PatchUsersUserId(w http.ResponseWriter, r *http.Request) {
	...
	result, err := c.service.PatchUsersUserId(r.Context(), userIdParam, userPatchParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
...
func (c *DefaultApiController) PostUser(w http.ResponseWriter, r *http.Request) {
	userDraftParam := UserDraft{}
	d := json.NewDecoder(r.Body)
	d.DisallowUnknownFields()
	if err := d.Decode(&userDraftParam); err != nil {
		c.errorHandler(w, r, &ParsingError{Err: err}, nil)
		return
	}
	if err := AssertUserDraftRequired(userDraftParam); err != nil {
		c.errorHandler(w, r, err, nil)
		return
	}
	result, err := c.service.PostUser(r.Context(), userDraftParam)
	// If an error occurred, encode the error with the status code
	if err != nil {
		c.errorHandler(w, r, err, &result)
		return
